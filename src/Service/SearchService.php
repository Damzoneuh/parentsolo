...
class SearchService
{
    ...
    public static function filterLookingFor(array $users, User $currentUser){
        ...
        /** @var User $user */
        foreach ($users as $user){
            if (null === $currentUser->getProfil()->getIsLookingFor()){
                array_push($result, $user);
            }
            switch ($currentUser->getProfil()->getIsLookingFor()){
                case null:
                    array_push($result, $user);
                    break;
                ...
                case true:
                case false:
                    if ($currentUser->getProfil()->getIsLookingFor() === $user->getProfil()->getIsMan()
                        || null === $currentUser->getProfil()->getIsLookingFor()){
                        array_push($result, $user);
                    }
                    break;
            }
